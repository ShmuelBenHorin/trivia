...
class Cfg {
  ...
  // Skippable ad after 5 seconds → +1 energy
  ...
  static const adRewardedEnergy     = 5;  // Energy from full ad
  static const adInterstitialEnergy = 1;  // Energy from skippable ad
  ...
  static const starsToUnlockMedium  = 0;
  static const starsToUnlockHard    = 0;
...
// ═══════════════════════════════════════════════
//  REPO — To add questions: edit questions_easy/medium/hard.dart
// ═══════════════════════════════════════════════
class QRepo {
...
class EnergyService extends ChangeNotifier {
  ...
  // No timer shown — keeping uncertainty like Duolingo
  String get label => '';
  bool get canWatchAd => _e < maxE && !PurchaseService.instance.isPremium;
  // Energy from ad — amount depends on ad type
...
// ═══════════════════════════════════════════════
class Sfx {
  // ✅ Correct — bright marimba just like Duolingo
  ...
  // ❌ Wrong — deep low "dunk" like Duolingo
  ...
  // 🏆 Perfect — short celebratory jingle, Duolingo streak style
  ...
  // 💀 Failed — three descending "dunks", like Duolingo losing a streak
...
class App extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    return ListenableBuilder(listenable:PurchaseService.instance,
      builder:(_,__)=>MaterialApp(title:'Trivia',debugShowCheckedModeBanner:false,
...
// ═══════════════════════════════════════════════
//  AD REWARD DIALOG — Two AdMob ad types
//  • Rewarded (30 seconds) → +5 energy
//  • Interstitial (skip after 5 seconds) → +1 energy
// ═══════════════════════════════════════════════
...
class _AdRewardDialogState extends State<_AdRewardDialog>
  ...
  // ── Load Rewarded ad (30 seconds, +5 energy) ──────────────────────────
  ...
  // ── Load Interstitial ad (skip after 5 seconds, +1 energy) ────────────
  ...
  void _showInterstitialAd() {
    ...
    ad.fullScreenContentCallback = FullScreenContentCallback(
      onAdDismissedFullScreenContent: (a) {
        ...
        // Give +1 even if skipped — user saw at least part of the ad
  ...
  Widget _mainView() => Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    // ── Button 2: Interstitial (skip after 5 seconds, +1) ──────────────
    _AdButton(
      ...
      SafeArea(child:Column(children:[
        ...
        Padding(padding:const EdgeInsets.fromLTRB(20,16,20,0),
          child:Row(children:[
            ShaderMask(shaderCallback:(b)=>const LinearGradient(colors:[Pal.gold,Color(0xFFFF9F0A)]).createShader(b),
              child:const Text('TRIVIA',style:TextStyle(fontSize:36,fontWeight:FontWeight.w900,color:Colors.white,letterSpacing:3))),
            const Spacer(),
            const EnergyChip(),
          ])),
        const SizedBox(height:12),
        ...
        Expanded(child:SingleChildScrollView(padding:const EdgeInsets.symmetric(horizontal:20),child:Column(children:[
          ...
          Align(alignment:Alignment.centerLeft,
            child:Text('🎯 Category Quiz',style:TextStyle(color:Pal.gold,fontSize:15,fontWeight:FontWeight.w800))),
          const SizedBox(height:10),
  ...
  showModalBottomSheet(context:ctx,backgroundColor:Colors.transparent,builder:(_)=>
    Container(
      ...
      child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        const Text('Select difficulty',style:TextStyle(color:Pal.ts,fontSize:13)),
        const SizedBox(height:20),
        ...Diff.values.map((d){
                  ...
                  showModalBottomSheet(context:ctx,isScrollControlled:true,backgroundColor:Colors.transparent,
                    builder:(_)=>PaywallSheet(onCode:(c){
                      ...
                      ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
                        content:Text(ok?'Code approved! All features unlocked 🎉':'Invalid code'),
...
class _HomeCats {
  static const cats = [
    ...
    ('tv',        'Television',     '📺',  Color(0xFF9B59B6)),
    ('music',     'Music',          '🎵',  Color(0xFFE91E8C)),
    ('sports',    'Sports',         '⚽',  Color(0xFFE74C3C)),
    ('geography', 'Geography',      '🌍',  Color(0xFF2ECC71)),
    ('science',   'Science',        '🔬',  Color(0xFF3498DB)),
    ('world',     'World Culture',  '🎬',  Color(0xFFE67E22)),
  ];
}
...
class _DiffCard extends StatelessWidget {
    ...
    return ListenableBuilder(listenable:LevelService.instance,builder:(_,__){
      ...
      return GestureDetector(
        onTap:(){
          ...
          if(!unlocked){ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Need $need stars to unlock'),backgroundColor:Pal.red));return;}
          ...
          child:Row(children:[
            ...
            Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              ...
              Text(unlocked?'${QRepo.levelCount(diff)} levels · $earned/$maxS ⭐'
                :(isPrem?'Requires Pro + $need stars':'Need $need ⭐ to unlock'),
  ...
  void _paywall(BuildContext ctx){showModalBottomSheet(context:ctx,isScrollControlled:true,backgroundColor:Colors.transparent,builder:(_)=>PaywallSheet(onCode:(c){final ok=PurchaseService.instance.tryDev(c);Navigator.pop(ctx);ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content:Text(ok?'Code approved! All features unlocked 🎉':'Invalid code'),backgroundColor:ok?Pal.green:Pal.red));}));}
}
        ...
        SafeArea(child:Column(children:[
          Padding(padding:const EdgeInsets.fromLTRB(16,12,16,0),
            child:Row(children:[
              ...
              const SizedBox(width:12),
              Text('${diff.label} Mode',style:TextStyle(color:diff.color,fontSize:22,fontWeight:FontWeight.w800)),
              const Spacer(),
              const EnergyChip(),
            ])),
          ...
          child:Container(
            ...
            child:Row(mainAxisSize:MainAxisSize.min,children:[
              ...
              Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                Text('${nextDiff.label} Mode',style:TextStyle(color:unlocked?Pal.tp:Pal.ts,fontSize:15,fontWeight:FontWeight.w800)),
                Text(unlocked?'Tap to open ▶':'Need ${nextDiff==Diff.medium?Cfg.starsToUnlockMedium:Cfg.starsToUnlockHard} ⭐ to unlock',
    ...
    return SizedBox(height:110,child:Stack(children:[
      ...
      Align(alignment:align,child:GestureDetector(
        onTap:(){
          ...
          EnergyService.instance.spend(Cfg.energyCostWrong); // Entering a level costs 1 energy
      ...
      child:Scaffold(backgroundColor:Pal.bg,body:Stack(children:[
        ...
        SafeArea(child:Column(children:[
          ...
          Padding(padding:const EdgeInsets.fromLTRB(16,8,16,0),child:Row(children:[
            ...
            const SizedBox(width:12),
            Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              Text('Level ${_gs.levelIdx+1} · ${widget.diff.label}',style:const TextStyle(color:Pal.ts,fontSize:11,fontWeight:FontWeight.w600)),
              const SizedBox(height:4),
  ...
  @override Widget build(BuildContext context){
    return Dialog(backgroundColor:Pal.card,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(24)),
      child:Padding(padding:const EdgeInsets.all(24),child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        const Text('Leave the level?',style:TextStyle(color:Pal.tp,fontSize:20,fontWeight:FontWeight.w800)),
        const SizedBox(height:8),
        const Text('Your progress in this level will not be saved.',textAlign:TextAlign.center,style:TextStyle(color:Pal.ts,fontSize:14)),
        const SizedBox(height:24),
        Row(children:[
          Expanded(child:OutlinedButton(onPressed:()=>Navigator.pop(context,false),
            ...
            child:const Padding(padding:EdgeInsets.symmetric(vertical:12),child:Text('Keep Playing',style:TextStyle(color:Pal.accent,fontWeight:FontWeight.w700))))),
          const SizedBox(width:12),
      ...
      SafeArea(child: Center(child: SingleChildScrollView(
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          FadeTransition(opacity: _enter,
            child: Text(perfect ? 'Perfect! 🌟' : 'Well done!',
              style: TextStyle(color: perfect ? Pal.gold : Pal.tp,
                fontSize: 30, fontWeight: FontWeight.w900))),
          const SizedBox(height: 6),
          Text('Level ${widget.levelIndex + 1} · ${widget.diff.label}',
          ...
          AnimatedOpacity(
            ...
            child: AnimatedSlide(
              ...
              child: Column(children: [
                if (_canNext) ...[
                  _bigBtn('Next Level ▶', widget.diff.color, () =>
  ...
  @override Widget build(BuildContext context){
    return Scaffold(backgroundColor:Pal.bg,body:Stack(children:[
      ...
      SafeArea(child:Center(child:SingleChildScrollView(padding:const EdgeInsets.all(32),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
        ...
        const SizedBox(height:16),
        FadeTransition(opacity:_c,child:const Text('You Failed!',style:TextStyle(color:Pal.red,fontSize:36,fontWeight:FontWeight.w900))),
        const SizedBox(height:8),
        Text('Level ${widget.levelIndex+1} · ${widget.diff.label}',style:const TextStyle(color:Pal.ts,fontSize:16)),
        const SizedBox(height:20),
        ...
        const SizedBox(height:48),
        _bigBtn('\u{1F504}  Try Again',widget.diff.color,(){
          if(!EnergyService.instance.has){Navigator.push(context,_slide(const NoEnergyScreen()));return;}
          Navigator.pushReplacement(context,_slide(GameScreen(diff:widget.diff,levelIndex:widget.levelIndex)));
        }),
        const SizedBox(height:14),
        _outBtn('\u{1F5FA}\uFE0F  Level Map',()=>Navigator.popUntil(context,(r)=>r.isFirst)),
      ])))),
    ]));
  }
}
  ...
  @override Widget build(BuildContext context){
    return Dialog(backgroundColor:Pal.card,shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(24)),
      child:Padding(padding:const EdgeInsets.all(24),child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        const Text('Restart from the beginning?',style:TextStyle(color:Pal.tp,fontSize:20,fontWeight:FontWeight.w800)),
        const SizedBox(height:8),
        const Text('The level will restart. Earned stars will be kept.',textAlign:TextAlign.center,style:TextStyle(color:Pal.ts,fontSize:14)),
        const SizedBox(height:24),
        Row(children:[
          Expanded(child:OutlinedButton(onPressed:()=>Navigator.pop(context,false),
            style:OutlinedButton.styleFrom(side:const BorderSide(color:Pal.accent),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(14))),
    ...
    return Scaffold(backgroundColor:Pal.bg,body:Stack(children:[
      ...
      SafeArea(child:Column(children:[
        ...
        Expanded(child:Center(child:SingleChildScrollView(padding:const EdgeInsets.all(28),child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
          ...
          const SizedBox(height:16),
          FadeTransition(opacity:_c,child:const Text('Out of Energy!',
          ...
          Container(
            ...
            child:Column(children:[
              ...
              const SizedBox(height:14),
              Text(
                'Every 15 minutes, ${isPro?Cfg.energyRechargeAmtPro:Cfg.energyRechargeAmt} energy is added',
              ...
              if(e.label.isNotEmpty)...[
                ...
                Container(
                  ...
                  child:Text('Next recharge: ${e.label}',
          ...
          if(!isPro)...[
            Container(
              ...
              child:Column(children:[
                const Text('\u{1F451}  Get Pro',
                  style:TextStyle(color:Pal.premium,fontSize:18,fontWeight:FontWeight.w900)),
                const SizedBox(height:10),
                Text(
                  'Pro recharges ${Cfg.energyRechargeAmtPro} energy every 15 minutes\ninstead of ${Cfg.energyRechargeAmt} in free mode',
                  textAlign:TextAlign.center,
                    ...
                    showModalBottomSheet(context:context,isScrollControlled:true,backgroundColor:Colors.transparent,
                      builder:(_)=>PaywallSheet(onCode:(c){
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                          content:Text(ok?'Code approved! 🎉':'Invalid code'),
                          backgroundColor:ok?Pal.green:Pal.red));
                      }));
                  },
...
class _PS extends State<PaywallSheet>{
  ...
  final _lbl=['Monthly','Yearly','Lifetime'];
  final _price=['\$3.99/mo','\$19.99/yr','\$14.99'];
  final _save=['Most Popular','Save 58%','Forever'];
    ...
    return Container(height:MediaQuery.of(context).size.height*0.85,
      ...
      child:Column(children:[
        ...
        Expanded(child:SingleChildScrollView(padding:const EdgeInsets.fromLTRB(24,20,24,0),child:Column(children:[
          ...
          const SizedBox(height:16),
          const Text('Trivia Pro',style:TextStyle(color:Pal.tp,fontSize:28,fontWeight:FontWeight.w900)),
          const SizedBox(height:18),
          _bf('\u{1F534}','Hard levels unlocked'),
          _bf('\u26A1','50 energy (4x more)'),
          _bf('\u{1F6AB}','No ads'),
          _bf('\u{1F513}','Access to all future content'),
          const SizedBox(height:24),
          Row(children:_planTiles()),
          const SizedBox(height:16),
          GestureDetector(onTap:()=>setState(()=>_showCode=!_showCode),child:Text('Have an access code?',style:TextStyle(color:Pal.ts.withOpacity(0.6),fontSize:12,decoration:TextDecoration.underline))),
          if(_showCode)...[const SizedBox(height:12),Row(children:[
            Expanded(child:TextField(controller:_ctrl,style:const TextStyle(color:Pal.tp),decoration:InputDecoration(hintText:'Enter code...',hintStyle:const TextStyle(color:Pal.ts),filled:true,fillColor:Pal.card,border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)),contentPadding:const EdgeInsets.symmetric(horizontal:16,vertical:12)))),
            const SizedBox(width:10),
            GestureDetector(onTap:()=>widget.onCode(_ctrl.text.trim()),child:Container(padding:const EdgeInsets.symmetric(horizontal:16,vertical:14),decoration:BoxDecoration(color:Pal.accent,borderRadius:BorderRadius.circular(12)),child:const Text('Apply',style:TextStyle(color:Colors.white,fontWeight:FontWeight.w900)))),
          ])],
          const SizedBox(height:20),
        ]))),
        Padding(padding:EdgeInsets.fromLTRB(24,0,24,MediaQuery.of(context).padding.bottom+16),child:Column(children:[
          ...
          const SizedBox(height:10),
          GestureDetector(onTap:()async{final ok=await ps.restore();if(mounted){Navigator.pop(context);if(ok)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('✅ Purchase restored!'),backgroundColor:Pal.green));}},
            child:const Text('Restore purchases',style:TextStyle(color:Pal.ts,fontSize:13,decoration:TextDecoration.underline))),
          const SizedBox(height:8),
          const Text('Cancel anytime · Billed via App Store or Google Play',textAlign:TextAlign.center,style:TextStyle(color:Pal.ts,fontSize:11)),
        ])),
      ]));
  }
        ...
        SafeArea(child: Column(children: [
          Padding(padding: const EdgeInsets.fromLTRB(16,12,16,0),
            child: Row(children: [
              _iconBtn(Icons.arrow_back, () => Navigator.pop(context)),
              const SizedBox(width:12),
              const Text('Category Quiz', style: TextStyle(
...
class _CQState extends State<CategoryQuizScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Entering a quiz costs 1 energy
                ...
                AnimatedOpacity(opacity:_fb?1:0,duration:const Duration(milliseconds:250),
                  child:Container(
                    ...
                    child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
                      ...
                      const SizedBox(width:10),
                      Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                        Text(_sel==_cur.c?'Correct!':'Answer: ${_cur.a[_cur.c]}',
...
// Result screen for category quiz
...
class _ResultViewState extends State<_ResultView> with TickerProviderStateMixin {
  ...
  String get _msg => {'S':'Perfect! True genius 🏆',
    'A':'Excellent! Almost perfect 🎯',
    'B':'Well done! 💪',
    'C':'Not bad, you can do better 📖',
    'D':'Keep practicing 💡'}[_grade]!;
    ...
    return Scaffold(backgroundColor:Pal.bg, body:Stack(children:[
      ...
      SafeArea(child:Center(child:SingleChildScrollView(
        ...
        child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[
          ...
          const Text('POINTS',style:TextStyle(color:Pal.ts,letterSpacing:3,fontSize:12)),
          const SizedBox(height:24),
          // Stats row
          Row(children:[
            _stat('✅ Correct','${widget.correct}/${widget.total}',Pal.green),
            const SizedBox(width:12),
            _stat('🔥 Best Streak','${widget.bestStreak}x',const Color(0xFFFF6B00)),
            const SizedBox(width:12),
            _stat('🎯 Accuracy',
          ...
          AnimatedOpacity(opacity:_showBtns?1:0,duration:const Duration(milliseconds:400),
            child:AnimatedSlide(
              ...
              curve:Curves.easeOut,
              child:Column(children:[
                _bigBtn('🔄  Play Again',widget.color,widget.onPlayAgain),
                const SizedBox(height:12),
                _outBtn('🏠  Back',widget.onBack),
              ]))),
        ])))),
    ]));
  }
