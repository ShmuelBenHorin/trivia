...
class Cfg {
  ...
  static const segmentSize          = 5;   // levels per segment
  static const starsPerSegment      = 10;  // avg 2 stars × 5 levels to unlock next segment
...
// ═══════════════════════════════════════════════
//  REPO — להוסיף questions: ערוך questions_easy/medium/hard.dart
// ═══════════════════════════════════════════════
class QRepo {
...
class EnergyService extends ChangeNotifier {
  ...
  String get label {
    ...
    if (diff.inMinutes < 1) return 'Less than a min';
...
// ═══════════════════════════════════════════════
class Sfx {
  // ✅ Correct — מרימבה בהירה בדיוק כמו Duolingo
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: 4),
                  Text('Mistakes', style: TextStyle(color: Pal.red, fontSize: 12, fontWeight: FontWeight.w700)),
                ]))),
  ...
  showModalBottomSheet(context:ctx,backgroundColor:Colors.transparent,builder:(_)=>
    Container(
      ...
      child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        Text('Choose Difficulty',style:const TextStyle(color:Pal.ts,fontSize:13)),
        const SizedBox(height:20),
        ...Diff.values.map((d){
          ...
          SizedBox(height:26,child:Center(child:
            widget.isNext
              ?AnimatedBuilder(animation:_pulse,builder:(_,__)=>Container(
                  ...
                  child:const Text('▶  PLAY',style:TextStyle(color:Colors.white,fontSize:10,fontWeight:FontWeight.w800,letterSpacing:0.5))))
          ...
          if(widget.lockLabel.isNotEmpty)...[
            ...
            Container(
              ...
              child:Column(mainAxisSize:MainAxisSize.min,children:[
                ...
                Text('to unlock',textAlign:TextAlign.center,
...
class MistakesScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Pal.bgD,
        title: const Text('❌  Recent Mistakes', style: TextStyle(color: Pal.tp, fontWeight: FontWeight.w800)),
      ...
      body: list.isEmpty
        ? const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            SizedBox(height: 12),
            Text('No mistakes yet!', style: TextStyle(color: Pal.tp, fontSize: 18, fontWeight: FontWeight.w700)),
            SizedBox(height: 6),
            Text('Keep it up', style: TextStyle(color: Pal.ts, fontSize: 14)),
          ]))
        : ListView.builder(
            ...
            if(ps.isLoading)
              ...
              onTap:()async{
                ...
                if(ps.packages.isEmpty){
                  setState(()=>_errMsg='Could not load products. Check your connection and try again.');
                  return;
                }
...
class _ResultViewState extends State<_ResultView> with TickerProviderStateMixin {
  ...
  String get _msg => {'S':'Perfect! A true genius 🏆',
    'A':'Excellent! Almost perfect 🎯',
    'B':'Well Done! 💪',
    'C':'Not bad, can do better 📖',
